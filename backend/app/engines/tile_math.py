...
def tile_count(
    ...
    """
    calc = area_count(float(room_l) * float(room_w), tile_l, tile_w, waste_pct)
    calc["layout"] = layout_preview(room_l, room_w, tile_l, tile_w)
    return calc


def area_count(area: float, tile_l: float, tile_w: float, waste_pct: float) -> dict:
    """Area-method count for a given floor area (m²); shared by single and mixed paving."""
